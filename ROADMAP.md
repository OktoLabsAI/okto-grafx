...
| 3 | FP-3 | Qualified entity UNION output, polymorphic patterns and named variable-length paths | In progress: composed polymorphic reads and [native detached node/edge/one-hop path output](docs/ENTITY_VALUES.md), including entity UNION, nested aggregate spill, exported subquery bindings, pending identity/cursors/sort/DISTINCT; generalized named paths/type alternatives, access-path work, remaining TCK discrepancies and Pulse migration pending |
