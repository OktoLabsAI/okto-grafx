...
def _commit_one(stack: Stack, page_index: int, payload: bytes = b"row") -> int:
    ...
    txn.owner._stage_page_image(
        txn, HEAP, page_index, make_page_image(stack.codec, [payload], page_index=page_index)
    )
...
def test_a_gap_completed_publication_is_never_own(
    ...
    foreign = build_stack(database_root, owner_id="participant-b", wal_factory=_real_wal)
    ...
    gap_token_calls = [own for token, own in calls if token == committed]
