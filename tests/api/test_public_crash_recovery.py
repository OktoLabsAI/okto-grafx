...
def _cross_statement_graph_crash(
    aim: str,
) -> tuple[MemoryStorageDevice, FaultInjectingStorageDevice, int, SimulatedCrash]:
    """Cut a transaction that builds a whole small graph, at one write point.

    Two nodes and the edge that joins them, in three statements of ONE transaction: the edge
    names both endpoints by the private identities their inserts were given, and the commit
    resolves them. Nothing here is committed beforehand, so the graph exists only as staged work
    right up to the moment the process stops.
    """
    memory = MemoryStorageDevice(page_size=PAGE_SIZE)
    fault: FaultInjectingStorageDevice = (
        _CrashBeforeTheLogAccepts(memory)
    ...
    published_before = _published(memory).last_committed_lsn
    txn = crashed.begin("write")
    txn.execute(f"CREATE (:{TABLE} {{id: 1, name: 'source'}})")
    txn.execute(f"CREATE (:{TABLE} {{id: 2, name: 'target'}})")
    txn.execute(
...
def test_a_crashed_cross_statement_graph_publishes_all_of_it_or_none_of_it() -> None:
    """Nodes and the edge between them are one commit, and a crash cannot split them.

    The endpoints are the reason this needs its own gate. They are resolved from private
    identities inside the commit, so a crash mid-way is the one moment a HALF-resolved graph
    could reach the file: a node published without the edge that named it, or an edge published
    against an identity its node never received.
    """
    # Cut BEFORE the log accepts anything: none of the three rows exists.
    memory, fault, before, stopped = _cross_statement_graph_crash("log")
    assert stopped.file is not None and stopped.file.startswith("wal/")
    with _connect(fault, namespace=memory) as reopened:
        assert _published(memory).last_committed_lsn == before
        assert _surviving(reopened) == ()
        assert _live_edges(reopened) == ()
        # The graph is what this asserts, and there is none of it. A process that died mid-commit
        # also leaves the pages it had already allocated, and the verifier says so -- that is the
        # state a clean close settles, not damage to a row or an index, so the claim here is that
        # NOTHING ELSE is reported rather than that nothing is.
        assert {
            finding.kind for finding in reopened.verify("all").findings
        } <= {"page_unwritten"}

    # Cut AFTER the COMMIT record is durable: the next open completes all three together.
    memory, fault, before, stopped = _cross_statement_graph_crash("write_page")
    assert stopped.file == "heap.dat"
    assert _published(memory).last_committed_lsn == before  # durable, not yet published
    with _connect(fault, namespace=memory) as recovered:
        assert recovered.recovery_report.records_replayed > 0
        assert _published(memory).last_committed_lsn > before
        assert _surviving(recovered) == (1, 2)
        # The endpoints are the identities the two nodes actually received -- not a token, not a
        # zero, and not a number belonging to some other row.
        identities = {
            version.values[0]: version.record_id
            for _ref, version in recovered._heap.scan_all(
                recovered.catalog.catalog.table(TABLE)
            )
            if version.live
        }
        assert _live_edges(recovered) == ((identities[1], identities[2]),)
        assert recovered.verify("all").findings == ()
