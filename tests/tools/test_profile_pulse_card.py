...
def test_attach_confirmation_is_exact_and_fail_closed() -> None:
    ...
    assert (
        _await_profiler_attach(
            _AttachedProfiler(), messages, timeout_seconds=0.1  # type: ignore[arg-type]
        )
        >= 0
    )
    ...
    with pytest.raises(ProfileRefused, match="unexpected pre-attach"):
        _await_profiler_attach(
            _AttachedProfiler(), unexpected, timeout_seconds=0.1  # type: ignore[arg-type]
        )
...
def test_runner_source_has_no_public_pid_or_locals_escape_hatch() -> None:
    ...
    assert '"--locals",' in source  # an explicit forbidden-token assertion, not an option
