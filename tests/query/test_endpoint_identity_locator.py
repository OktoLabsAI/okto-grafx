...
from okto_grafx.engine.query_engine import (
    _ENDPOINT_LOCATOR_MEMO_BYTES,
    _ENDPOINT_LOCATOR_TABLE_BYTES,
    _EndpointLocatorCapacity,
...
def test_cycle_proof_survives_cursor_resumption_and_is_released_on_failure() -> None:
    ...
    assert stack.engine._endpoint_budget._used_bytes == _ENDPOINT_LOCATOR_MEMO_BYTES
    assert stack.engine._endpoint_budget._used_entries == 1
    stack.engine.settle_schema(1, committed=False)
    assert stack.engine._endpoint_budget._used_bytes == 0
    assert stack.engine._endpoint_budget._used_entries == 0
...
def test_capacity_discards_the_partial_locator_and_uses_canonical_lookup(
    ...
    retained = _ENDPOINT_LOCATOR_MEMO_BYTES + _ENDPOINT_LOCATOR_TABLE_BYTES
    stack.engine._endpoint_budget = _EndpointLocatorBudget(
        max_bytes=retained,
        max_entries=2,
    ...
    assert memo.is_disabled(stack.table("Person").table_id)
    assert len(memo.locators) == 1
    assert stack.engine._endpoint_budget._used_bytes == retained
    assert stack.engine._endpoint_budget._used_entries == 2
    stack.engine.settle_schema(1, committed=False)
    assert stack.engine._endpoint_budget._used_bytes == 0
    assert stack.engine._endpoint_budget._used_entries == 0


def test_saturation_cannot_grow_an_unaccounted_transaction_registry() -> None:
    stack = build_query_stack()
    ref = _insert_people(stack, 1)[0]
    stack.engine._endpoint_budget = _EndpointLocatorBudget(
        max_bytes=_ENDPOINT_LOCATOR_MEMO_BYTES - 1,
        max_entries=10_000,
        guard=stack.engine._endpoint_guard,
    )
...
def test_txn_registry_allocation_failure_returns_its_prepaid_budget() -> None:
    stack = build_query_stack()
    _insert_people(stack, 1)
    stack.engine._endpoint_memo = _RefusingMemoMap()

    with pytest.raises(MemoryError, match="mapping allocation"):
        _visible_identity_with_ref(
            stack.engine, _context(stack), stack.table("Person"), 1
        )
...
def test_table_registry_allocation_failure_returns_its_prepaid_budget() -> None:
    stack = build_query_stack()
    _insert_people(stack, 1)
    context = _context(stack)
    memo = _endpoint_txn_memo(stack.engine, context)
    assert memo is not None
    memo.locators = _RefusingMemoMap()  # type: ignore[assignment]

    with pytest.raises(MemoryError, match="mapping allocation"):
        _visible_identity_with_ref(
            stack.engine, context, stack.table("Person"), 1
        )
...
def test_the_injected_budget_guard_makes_competing_reservations_atomic() -> None:
    guard = _ObservedRLock()
    budget = _EndpointLocatorBudget(max_bytes=3, max_entries=3, guard=guard)
    ...
    accepted = sum(outcomes.get_nowait() for _ in threads)
    assert accepted == 3
...
def test_settlement_can_retire_a_building_memo_without_locking_heap_io(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    stack = build_query_stack()
    ref = _insert_people(stack, 1)[0]
    ...
    original = inspect.getattr_static(HeapStore, "_visible_record_cursor")
    ...
    def paused_cursor(
        ...
        *,
        ...
        assert not guard.held_here(), "heap I/O ran inside the endpoint registry guard"
        entered.set()
        assert proceed.wait(5)
        return original(store, table, snapshot, admit_page=admit_page)

    monkeypatch.setattr(HeapStore, "_visible_record_cursor", paused_cursor)
    context = _context(stack)
...
def _foreign_update(root: str, result: object) -> None:
    """Commit a replacement version while the parent keeps its old snapshot open."""
    try:
        with okto_grafx.connect(root) as database:
            with database.begin("write") as writer:
                writer.execute("MATCH (p:Person {id: 1}) SET p.name = 'foreign'")
        result.put(None)
    except BaseException as failure:  # pragma: no cover - the parent renders the child failure
        result.put(repr(failure))
...
def test_live_locator_falls_back_after_foreign_update_and_keeps_its_old_snapshot(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "live-locator"
    with okto_grafx.connect(root) as database:
        assert database._queries._endpoint_guard is not database._pool._guard
        with database.begin("write") as schema_txn:
            schema_txn.execute(
                "CREATE NODE TABLE Person(id INT64, name STRING, PRIMARY KEY(id))"
            )
        with database.begin("write") as seed:
            seed.execute("CREATE (:Person {id: 1, name: 'original'})")

        reader = database.begin("read")
        assert reader.execute("RETURN 1").rows == ((1,),)
        schema = database._catalog.catalog
        ...
        first = _visible_identity_with_ref(database._queries, context, table, 1)
        assert first is not None and first[1].values[1] == "original"
        memo = database._queries._endpoint_memo[reader.txn_id]
        assert len(memo.locators) == 1

        spawn = multiprocessing.get_context("spawn")
        result = spawn.Queue()
        process = spawn.Process(target=_foreign_update, args=(str(root), result))
        process.start()
        process.join(30)
        if process.is_alive():
            process.terminate()
            process.join(5)
            pytest.fail("the foreign updater did not terminate")
        assert process.exitcode == 0
        assert result.get(timeout=5) is None

        calls = 0
        original_lookup = inspect.getattr_static(HeapStore, "_lookup_with_ref")
        ...
        def counted_lookup(
            ...
            nonlocal calls
            calls += 1
            return original_lookup(store, target, record_id, snapshot)

        monkeypatch.setattr(HeapStore, "_lookup_with_ref", counted_lookup)
        before_reconcile = _visible_identity_with_ref(
            database._queries, context, table, 1
        )
        assert before_reconcile is not None
        assert before_reconcile[0] == first[0]
        assert before_reconcile[1].values[1] == "original"
        assert calls == 0, "the still-live old view reused its locator before reconciliation"

        with database._transactions.page_access_section(fresh_read_view=True):
            found_old = _visible_identity_with_ref(
                database._queries, context, table, 1
            )

        assert found_old is not None and found_old[1].values[1] == "original"
        assert found_old[0] == first[0]
        assert calls == 1, "the stale derived walk used the canonical snapshot-aware door"
        assert database._queries._endpoint_memo[reader.txn_id].locators == {}
        reader.rollback()
