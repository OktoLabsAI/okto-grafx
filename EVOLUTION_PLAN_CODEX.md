...
- **M-7 WAL-only re-medido e aprovado; seu escopo não foi inflado.** Em `origin/main@530df34`, o
  ...
  congelado é F1 de dois processos para preparar a medição CE-3. F1 permanece em rework, sem SHA
  aceito; `600d17b` e `1eec9e4` são commits provisórios e não certificam resultado. A ordem vinculante
  desta frente é **F1 aceito -> CE-3 literal em dois processos -> decisão CE-3 -> M-PULSE-7 10k**.
