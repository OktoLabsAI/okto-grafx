...
- **Compatibilidade Okto Pulse: M-PULSE-1 a M-PULSE-6 concluídos e certificados conforme o quadro
  ...
  como repetição de baseline/controle de ruído e não é evidência do patch. A rodada oficial C em
  `per_family=5` é o próximo passo finito. O plano/harness conjunto foi certificado no handoff
...
cross-process já prevista. C-community foi medida e reduziu o catálogo de 94 para 12 snapshots no
perfil curto, com ganho RAW agregado de `1,119–1,136x`. O Patch 2 não é promovido antes da rodada
oficial C `per_family=5` e da decisão explícita sobre o residual, evitando abrir outra frente sem
evidência.
...
| M-PULSE-7 — estabilização de performance A1/B/C | em execução; A1+B certificadas, integradas e publicadas em `main`; C-community implementada, revisada, medida e publicada no candidato Pulse; rodada oficial C `per_family=5` é o próximo gate | base Grafx `6d9b7a1`; A1 `5002a77c8d4e59e137890a45bcf61874398d2dcc`; B original `420ca4886e5e226aa10bf3a28a1396f98471faca`; código Grafx integrado `91a59c6806659cdd75d1b33500885f0c7354de6b`; candidato Grafx `origin/main@84b88aadfa68345be8269537b66c8d0849fb5ddd`; C-community `65d07b5` + hardening `c05d89d`, publicado em `origin/milestone/grafx-mpulse7-rollout@c05d89d`; handoffs B `hof_7a5be05b5b644b609b92b5e8c09fdeea`, harness `hof_1ad44f9d92d04f58abbcfe4a6e316ba6`, fixture `hof_42a66b0033f0454ca5e12b3f27668141`, runbook `hof_93dbf437246040a3a83cb900f3a488ea` concluídos/PASS; C `hof_d301014276c148c59c4709a2c2a87950` cancelado após entregar o commit/mutantes, follow-up concluído pelo Codex | A1 remove o fan-out de page 0 entre tabelas com freshness fail-closed; B restringe o namespace e preserva contenção contra redirects; C usa uma view pública de catálogo por scope com invalidação DDL fail-closed e preserva operações de erro. Gates C: cinco mutantes mortos, 213 testes relacionados no autor, 98/98 independentes, 9/9 pós-fast-forward e checks estáticos verdes. Evidência curta A1+B+C (`94c692afc859531119b5d73c38041e37f137bd638bc5467ec6cf2d9c59731eac`): 12/12, mesmo `operation_set_sha256=61e21ccf…`, forense e 68/43 page/index writes; catálogo 94→12 chamadas e `5.591,6–5.891,2`→`705,5 ms`; RAW `19.266,1–19.564,1`→`17.219,4 ms`; instrumentado `40.017,2–42.218,0`→`34.540,5 ms`. A rodada A1+B `per_family=5` versus Ladybug permanece a referência pré-C: `18.561,15` versus `970,62 ms` (`19,12x`), ainda fora de D5 `<=10x` e meta final `<=1x`; o manifesto M-PULSE-7 não impõe SLO. Rodada oficial C decide o residual antes de qualquer Patch 2; A2/plan cache continuam não promovidos. Quarentenas e RUN #4 permanecem inalterados |
