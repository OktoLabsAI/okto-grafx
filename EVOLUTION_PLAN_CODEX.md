...
| 0.0.2 / lote de escala 21 — identidade final no-follow | concluído e auditado na branch | `b5cff4a`; revisão independente PASS; detalhes em `docs/PERFORMANCE_ROUND_0_0_2.md` | A prova obrigatória de namespace passa a devolver o `lstat` fresco do componente final para a comparação com `fstat`, removendo exatamente um `stat(path)` redundante por revalidação. O walk no-follow integral, o fallback com containment, os boundaries strict/generation, a detecção de replacement e recusas de redirect/junction/reparse permanecem. Teste estrutural `stat 1→0` por warm hit; sem cache entre chamadas e sem alegação de redução de `lstat` ou speedup amplo. Formato, WAL/OCC, durabilidade, writer lease e multiwriter/multireader intactos |
