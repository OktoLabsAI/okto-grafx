...
- **Linha `0.0.2` iniciada sob o plano de performance congelado.** A branch canônica de trabalho é
  ...
  estão publicados, e o head atual é `27c2379`. A auditoria do Pulse Community pinado em `d50c034`
  ...
  blocker do fluxo real. D-26, D-01 e D-04 estão desenvolvidos e testados em branches isoladas,
  aguardando P0.3/P0.4 para decisão de promoção; o protótipo D-02 foi descartado porque poderia
  ocultar duplicata corrupta. P0.2 está concluído: as primitivas fail-closed entraram em `c276dec` e
  ...
  localidade dinâmica de endpoints, atividade vetorial e censo agregado read-only; a regressão
  combinada passou 98/98. A execução real de P0.3 e as séries P0.4 continuam aguardando o término
  do backfill vivo e usarão somente cópias autenticadas. A
...
| Roadmaps complementares pós-Pulse | incorporados por referência; gate `0.0.1` cumprido e linha `0.0.2` iniciada pela rodada delimitada em `GRAFX_PERFORMANCE_ROUND_FINAL.md` | `feature/v0.0.2@27c2379`; P0.2 originado em `perf/v002-p0-instruments-claude@9635146` e `perf/v002-p0-card-driver@e8a6be0`; P0.3 originado em `perf/v002-p0-census@7cdb204` e integrado em `8ab6a8b`; `GRAFX_COMPLEMENTARY_EVOLUTION_PLAN_CODEX.md` (`GX-CAP-0..11`, `GX-AGENT-0/1`) e `AGENT_FIRST_EVOLUTION_PLAN_CODEX.md` (`AGENT-0..8`) | P0.0/P0.1/P0.2 estão concluídos. O ferramental P0.3 — perfil protegido, localidade/atividade dinâmica e censo read-only reconciliado — passou regressão combinada 98/98; sua execução real e P0.4 aguardam o término do backfill vivo. D-26, D-01 e D-04 estão desenvolvidos em branches isoladas e não serão promovidos antes da medição prevista. Ambos os roadmaps integrais continuam autoridades versionadas depois desta rodada: database-first governa ownership/ordem no core; agent-first preserva todos os requisitos e gates detalhados da camada opcional. Nenhum item amplia retroativamente o lote de performance; sobreposição usa o conjunto compatível mais estrito e conflito exige ADR explícita |
