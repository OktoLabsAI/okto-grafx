...
| 0.0.2 / lote de escala 38 — fatos RESET reaproveitados das validações obrigatórias | concluído na branch; revisão Nexus solicitada | `6b142ba`; Nexus `hof_a85992ce987045a7ac73b2bb42ac822c`; detalhes em `docs/PERFORMANCE_ROUND_0_0_2.md` | O preflight integral e o validator de staging passam a transportar o fato `contains_index_reset` já decodificado, removendo dois rescans lineares. O checkpoint só consome proof verificado por seal/owner/replay/passage/signature e cai no replay canônico em qualquer dúvida. O commit só confia no manager/validator built-in exatos e continua revalidando após retarget. RESET é identificado pela operação, não pelo tipo WAL. A/B direcional do cenário 2.000/8/8 mediu `~1,02x`; testes de payload corrompido e provas incompatíveis passaram. Sem novo gate ou alteração de WAL/OCC, durabilidade e concorrência |
