...
- **Compatibilidade Okto Pulse: M-PULSE-1 a M-PULSE-5 concluídos, publicados e auditados conforme
  ...
  `milestone/grafx-mpulse6-integration@8f54dc1` (publicada como milestone, ainda não promovida): bindings
  ...
  pool/transação. Facades roteadas, composição integral, quarantine/restore de diretório Grafx,
  bypasses remanescentes e a matriz end-to-end continuam abertos; portanto este SHA
...
| M-PULSE-6 — fundação, resolver imutável e pinning antes da composição | em execução; terceiro checkpoint publicado, sem promoção | Pulse Community `origin/milestone/grafx-mpulse6-integration@8f54dc1`; commits integrados `00247fc..8f54dc1`; resolver `559647b` + hardening P0 `6d0dc2` (origem revisada `b346ddc`); pool/pinning `fb21702` + correção terminal `8f54dc1`, entrega Claude `hof_dba65f737770434aa705f6f25475349a` concluída/verificada/PASS; engine LIMIT no Grafx `milestone/grafx-mpulse6-path-limit-claude@b3fd6e457505e18d1bdf30934d7fcf3767a9cd03` publicado apenas no branch de milestone | Além das provas anteriores, o resolver mantém o anchor Global Ladybug imutável, separa active generation, recusa `.wal`/`.shadow`/`.wal.checkpoint` órfãos Board/Global antes de opener/callback/binding e bloqueia root/pais junction antes de qualquer `mkdir`/`FileLock`. A revisão independente exercitou 12 combinações de resíduos e junctions Windows reais, com zero escrita externa, e fechou route+foundation 52/52. O pool valida `max_entries`, aplica o bound antes de connect, faz LRU apenas de entradas não pinadas, recusa all-pinned/close pinado, preserva vítimas/falhas rastreáveis e libera leases exatamente uma vez sob concorrência. A transação pina antes de `begin`, só libera após `active=False` em commit/rollback/factory failure e mantém commit durável não retryable se a release falhar. A entrega completa passou 112/112; revisão independente 51 pool + 16 transacionais; integração 103 resolver/foundation/pool + 15 finais transacionais, Ruff/format/diff-check PASS. O gate permanece aberto somente para facades roteadas, composição, quarantine/restore de diretório Grafx, bypasses remanescentes, conformance/end-to-end e regressão longa; nenhuma promoção é reivindicada por este checkpoint |
