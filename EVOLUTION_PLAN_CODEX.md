...
- **Compatibilidade Okto Pulse: M-PULSE-1 concluído; M-PULSE-2 em execução.** M-PULSE-0 foi concluído em
  ...
  concluído/verificado/PASS, assim como três auditorias independentes. O próximo alvo fixo é
  M-PULSE-2N, relação sem tipo de um hop, sem ampliar o gate já registrado em 9.7.
...
| Cypher read-only 1.0 | escalares, `CASE`, subscritos, `label`, `timestamp`, `UNWIND`, `WITH` não agregante, `MATCH (n)` polimórfico, endpoint inference tipada de I01/I02, named path decorativo, upper bound implícito, `OPTIONAL MATCH` root, convergência da autoridade pública e `UNION` binário concluídos até M-PULSE-2M | restam exatamente duas capabilities congeladas do engine: relação sem tipo e path projection | P1 |
