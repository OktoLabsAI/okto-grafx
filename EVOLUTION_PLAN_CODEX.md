...
|---|---|---|---|
| GX-CAP-1 / CAP-1B — páginas e lookup | planejador privado/reader implementados; ativação e integração pendentes | branch `feature/gx-cap-1`; `engine/commit_catalog_store.py`; formato e limites em `COMMIT_CATALOG_V1.md`; evidência em `SPEC-GX-CAP-1.md` | 59 testes específicos, incluídos em 606 testes agrupados em 14,79 s; append pequeno até 4 imagens/8 leituras em 1.000 commits, lookup com limite logarítmico; arquivos reais temporários, corrupção CRC-válida, imagens ausentes e replay byte-idempotente. Sem escrita independente, ativação de formato ou uso de specs do Pulse. Faltam staging/WAL/OCC/recovery/autoridade física e os demais requisitos do marco |
