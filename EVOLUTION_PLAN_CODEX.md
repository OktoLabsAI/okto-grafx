...
| M-PULSE-2K — `OPTIONAL MATCH` root de um nó | alvo fixo em implementação | baseline `main@7519bbd`; branch `milestone/pulse-query-optional-match`; handoff Nexus a registrar | Única forma admitida neste lote: primeira cláusula `OPTIONAL MATCH (v:Label)`, com variável nomeada, um label, nenhum mapa inline, relação, path ou pattern adicional, seguida apenas do `WHERE` e subconjunto de `RETURN` já suportados. Sem match produz exatamente uma linha com `v = NULL`; com match não acrescenta linha nula; `v.prop`/`label(v)` propagam `NULL`, `count(v)=0` e `count(*)=1`. Parser, analyzer e planner repetem o gate inclusive para AST/análise fornecida; snapshot, RYOW, rollback e budgets existentes são preservados. Chaining, uso após `MATCH`/`WITH`/`UNWIND`, escrita, nó anônimo/sem label/multilabel/com mapa, relações, named/path, `UNION`, relação sem tipo, `CALL` e homoglyph continuam recusados pré-stream. Alvo do ratchet: raw 74/13, cinco débitos; entries 82/13; digest `db3802aa449dc6a0204680c822027f8a8c6b0eeddef71afc350528b3c2d89e05`. Nenhum outro probe ou feature muda; a regressão longa será acumulada após este incremento |
