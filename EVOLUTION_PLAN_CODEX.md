...
- **Compatibilidade Okto Pulse: M-PULSE-1 concluído; M-PULSE-2 em execução.** M-PULSE-0 foi concluído em
...
fechados. `execute()` genérico permanece deliberadamente em M-PULSE-2.
...
| M-PULSE-2L — convergência da autoridade pública | alvo fixo; próximo em execução | depende de `main@1d3f2df`; Pulse Core parte de `ab61b9a`; branches isoladas `milestone/grafx-query-authority` no Core e a registrar no Grafx | A fronteira pública Core recusa `CALL`/`YIELD` como `unsupported_operation` quando aparecem depois de um root suportado, preservando `unsafe_cypher` para a blacklist, a taxonomia de `CALL` como root, a invisibilidade de comentários/literais e as rotas internas explicitamente allowlisted. O freezer normaliza NFKC somente o texto dos probes da superfície pública antes de consultar o engine; não normaliza famílias internas nem o parser público da biblioteca. Somente três probes podem mudar: homoglyph de root e homoglyph de escrita passam a planejar no engine; `CALL` pós-root passa de permitido para recusado no contrato. Ratchet esperado: engine 76/11, contrato 73/14, três débitos (`UNION`, relação sem tipo e path projection), entries 82/13. O novo SHA Core e o digest serão registrados somente depois dos commits e do `--check` |
| M-PULSE-2M — `UNION` binário simples | alvo fixo; bloqueado apenas por M-PULSE-2L | branch/SHAs a registrar | Exatamente duas branches read-only, cada uma terminada em `RETURN`, com mesma aridade e tipos compatíveis; nomes de coluna da primeira branch e distinct global. `UNION ALL`, chaining, nesting e qualquer write permanecem recusados. Snapshot/transação, cancelamento e budgets são combinados entre as duas branches. Somente o probe raw `UNION` pode mudar; ratchet esperado 77/10 e dois débitos restantes |
| M-PULSE-2N — relação sem tipo de um hop | alvo fixo; bloqueado apenas por M-PULSE-2M | branch/SHAs a registrar | Exatamente `MATCH (a:Decision)-[r]->(b) RETURN a.id`. O planner enumera deterministicamente todas as tabelas relacionais compatíveis por `table_id` e preserva multiplicidade; não escolhe só a primeira. Ranges, maps, path, `OPTIONAL`, `WITH`, writes, múltiplos patterns e outras direções permanecem recusados. Somente o probe raw de relação sem tipo pode mudar; ratchet esperado 78/9 e um débito restante |
| M-PULSE-2O — projeção exata de path | alvo fixo; bloqueado apenas por M-PULSE-2N e por um oracle Kuzu de forma pública | branch/SHAs a registrar | Exatamente `MATCH path = (a:Decision)-[r:supersedes]->(b:Decision) RETURN path`: um hop tipado e direcionado, todos os elementos nomeados e path como único item de retorno. Um único oracle diferencial no backend Kuzu atual congelará previamente a forma pública destacada; isso é pré-condição de compatibilidade, não expansão. Path genérico, multi-hop, `WITH`, agregação, filtros e mutações permanecem recusados. Somente o probe raw de path projection pode mudar; ratchet final esperado 79/8, zero débito permitido e todas as 73 consultas admitidas pelo contrato corrigido aceitas pelo engine |
