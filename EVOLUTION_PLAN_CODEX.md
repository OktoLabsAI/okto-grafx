...
- **Compatibilidade Okto Pulse: M-PULSE-1 concluído; M-PULSE-2 em execução.** M-PULSE-0 foi concluído em
  ...
  handoff Nexus `hof_9698ae9ce6574df881871be6bf98ede4` concluíram/PASS. O próximo alvo fixo é
  M-PULSE-2O, projeção exata de path. O oracle público Ladybug/Kuzu foi congelado e verificado no
  handoff Nexus `hof_baca7724b827446ebc78c4a1f27758ab`; a especificação anterior ao código está
  no commit `9458d6f00cc6a6cfcd5bc718fc58eeb45382786d`. A implementação está em curso sem ampliar a
  forma literal de 9.7.
...
| Cypher read-only 1.0 | escalares, `CASE`, subscritos, `label`, `timestamp`, `UNWIND`, `WITH` não agregante, `MATCH (n)` polimórfico, endpoint inference tipada de I01/I02, named path decorativo, upper bound implícito, `OPTIONAL MATCH` root, convergência da autoridade pública, `UNION` binário e relação sem tipo de um hop concluídos até M-PULSE-2N | resta exatamente uma capability congelada do engine: path projection | P1 |
...
| M-PULSE-2O — projeção exata de path | oracle concluído e verificado; implementação em execução | branch `milestone/pulse-query-path-projection`; spec pré-código `9458d6f00cc6a6cfcd5bc718fc58eeb45382786d`; oracle Nexus `hof_baca7724b827446ebc78c4a1f27758ab` concluído/verificado/PASS | Exatamente `MATCH path = (a:Decision)-[r:supersedes]->(b:Decision) RETURN path`: um hop tipado e direcionado, todos os elementos nomeados e path como único item de retorno. Ladybug 0.16.0 congelou `_NODES`/`_RELS`, ordem, propriedades, identidades opacas, correlações, zero/uma/paralelas e as variantes públicas canonical-only/include-working. Rewrite de camada e conversão tuple→list pertencem ao adapter, não ampliam este gate. Path genérico, multi-hop, `WITH`, agregação, filtros e mutações permanecem recusados. Somente o probe raw de path projection pode mudar; ratchet final esperado 79/8, zero débito permitido e todas as 73 consultas admitidas pelo contrato corrigido aceitas pelo engine |
