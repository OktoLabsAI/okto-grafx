...
|---|---|---|---|
| GX-CAP-1 / CAP-1B — binding do LSN final | preparação e retarget de imagens implementados; ligação ao coordenador pendente | branch `feature/gx-cap-1`; `CommitCatalogStore.prepare_append` / `PreparedCommitCatalogAppend.bind`; call sites e obrigações em `COMMIT_CATALOG_V1.md` | 471 testes agrupados em 15,72 s; cobertura da head deve coincidir com controle durável; metadata/tempo congelados, retarget sem I/O host, data/maintenance; planejador real do WAL com/sem roll sem append de efeitos ainda não suportados. Sem cache de autoridade, ativação, alteração das OCCs, deploy ou uso de specs |
