...
- **Compatibilidade Okto Pulse: M-PULSE-1 a M-PULSE-6 concluídos e certificados conforme o quadro
  ...
  `per_family=5` continua pendente para a rodada oficial. A
  frente B foi certificada no handoff Nexus `hof_7a5be05b5b644b609b92b5e8c09fdeea` e publicada em
  `origin/perf/b-storage-namespace@420ca4886e5e226aa10bf3a28a1396f98471faca`. Ela restringe
  ...
  reproduzida, recusada na revisão e fechada antes da certificação. O relatório Codex está
  congelado no commit `aa3de79f`. O plano/harness conjunto foi certificado no handoff
...
| M-PULSE-7 — estabilização de performance A1/B | em execução; A1 e B certificadas, commitadas e publicadas em branches; medição curta limpa de A1 concluída e promoção de A1 para `main` autorizada; B e rodada oficial pendentes | base Grafx `6d9b7a1`; A1 `origin/perf/a-index-freshness@5002a77c8d4e59e137890a45bcf61874398d2dcc`; B `origin/perf/b-storage-namespace@420ca4886e5e226aa10bf3a28a1396f98471faca`; handoff B `hof_7a5be05b5b644b609b92b5e8c09fdeea` concluído/verificado/PASS; harness `hof_1ad44f9d92d04f58abbcfe4a6e316ba6` e correção de fixture `hof_42a66b0033f0454ca5e12b3f27668141` concluídos/verificados/PASS; relatório Codex `aa3de79f` | A1 remove o fan-out de page 0 entre tabelas, deriva high-water somente de headers do heap e mantém fail-closed local/reopen/cross-process para índice omitido. Gates A1: regressão ampla 1.707/1.707, índices 202/202, query/public concurrency 311/311, nullable/vector visibility 29/29 e txn index-wiring/commit-protocol 65/65. Comparação curta limpa, mesmo conjunto lógico e 12/12 famílias: baseline `6d9b7a1`/`reopen` `4f828baf…` = `97,6 s` RAW e 1.949 páginas de índice; A1 `5002a77`/`continuous` `a749c2ee…` = `68,5 s` e 43 páginas (`~1,43x` wall, `~45,3x` menos writes de índice), com WAL idêntico por família. O smoke A1 antigo `e13a6899…` está QUARANTINED por fixture de projeção; a repetição corrigida passou integralmente. B reduz `list_files("wal/")` de `72 realpath/7 scandir` para no máximo dois `realpath` e um `scandir`, elimina `realpath` do hit quente e mantém `stat/fstat`; a revisão capturou e fechou antes do PASS uma fuga por junction NTFS entre a prova e o `scandir`. Gate B final: 1.185 passed/12 skips atribuídos no lote amplo do Claude, repetido pelo Codex com exit 0; junction real recusada, Ruff e diff-check verdes. A sonda `e722a3d...` é QUARANTINED: árvore contaminada, 7/12 famílias e nenhum baseline/RC0 aceito; somente a atribuição direcional RC2/RC3 em arquivos não modificados é reutilizável. RUN #4 foi parado por decisão explícita e não tem verdict; `per_family=5` e integração/medição de B permanecem pendentes |
