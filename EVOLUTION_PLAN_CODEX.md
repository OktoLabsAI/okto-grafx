...
| Cypher read-only 1.0 | escalares, `CASE`, subscritos, `label`, `timestamp`, `UNWIND`, `WITH` não agregante, `MATCH (n)` polimórfico, endpoint inference tipada de I01/I02, named path decorativo, upper bound implícito, `OPTIONAL MATCH` root e convergência da autoridade pública concluídos até M-PULSE-2L | restam exatamente três capabilities congeladas do engine: `UNION`, relação sem tipo e path projection | P1 |
...
| M-PULSE-2M — `UNION` binário simples | alvo fixo; próximo em execução | depende do merge M-PULSE-2L `main@c5ba889`; branch/SHAs a registrar | Exatamente duas branches read-only, cada uma terminada em `RETURN`, com mesma aridade e tipos compatíveis; nomes de coluna da primeira branch e distinct global. `UNION ALL`, chaining, nesting e qualquer write permanecem recusados. Snapshot/transação e budgets são combinados entre as duas branches; interrupções/exceções preservam a execução única sem resultado parcial, sem inventar neste lote o cancel token/deadline ainda inexistente na baseline. Somente o probe raw `UNION` pode mudar; ratchet esperado 77/10 e dois débitos restantes |
