...
| 0.0.2 / lote de escala 4 — vetor seletivo e identidade registrada | VEC-4/CAT-5 concluídos; EDGE reaberto com contrato congelado | `c77d414`, `40552df`, protótipo EDGE `b236697` retirado por `030b39d`; detalhes em `docs/PERFORMANCE_ROUND_0_0_2.md` | VEC-4 autentica candidatos exatos selados pelo engine por heap MVCC e bucket, com fallback canônico anterior ao ranking; caso focal 4/64 leituras e nenhum walk integral, sem alegar O(K). CAT-5 elimina abertura de header v2 já certificado (`5,8x` no allocator N=64; `1,37x` no DDL amostrado), preservando fallback legado fail-closed. A revisão EDGE provou que pushdown planner-only podia suprimir erro, corrupção e budget e permitir escrita antes recusada; o revert e 110 regressões preservam ordem, métricas e atomicidade. Próxima precedência é um operador dedicado que reduza trabalho físico mantendo `N+N²` lógico/budget; knobs HNSW ficam deferidos até perfil 8.192×384 e harness de recall adequado |
