...
| STAB-GRAFX-Q1 — boundary pura de `timestamp()` | alvo fixo em implementação antes de M-PULSE-2K | baseline `main@a32edc3`; branch `milestone/stabilize-timestamp-boundary`; handoff Nexus `hof_61849c800fbb46d7bbb9e20d0d18cb5d` | Uma única violação em `query_engine.py` é reportada por dois gates: o scalar introduziu `datetime` no core puro. A correção não amplia a allowlist nem toca Clock/cancelamento: substitui apenas a conversão privada por parser ISO/Gregoriano inteiro e determinístico, preservando a superfície efetivamente aceita, a taxonomia `GrafxPlanError`, a API e o corpus. Gate finito: boundary volta a zero; todos os testes timestamp existentes e matriz diferencial de calendário, semana ISO, pre-epoch, frações, offsets e entradas hostis passam; Ruff/format/diff-check limpos. Nenhum outro gap ou feature entra neste lote |
