# SPEC-GX-AGENT-1 — Optional integrated knowledge layer

Status: specified / implementation not certified. Date: 2026-09-08.
Dependencies: Temporal + full-text/hybrid; production lifecycle; GX-AGENT-0.

## Normative scope

[Full complementary requirements](../archive/ROADMAP_SOURCES.md#source-grafx-complementary-evolution-plan-codex), §16; §17 GX-AGENT-1; agent-first §§12–23 (AGENT-6..8 plus all prior contracts).
...
Deliverables: Historical recall/diff; bounded hybrid ContextPack; contradiction/supersession/retraction; scope promotion receipts; temporal claim validity; MCP capability negotiation; optional Nexus correlation. No automatic winner selection or prompt/transcript retention.

## Contract and implementation boundary

Binding decision: [ADR-GX-001-DATABASE-FIRST.md](../architecture/ADR-GX-001-DATABASE-FIRST.md).
Planned owning modules: Optional agent/MCP packages consuming implemented database capabilities; external model/tokenizer providers optional.
...
unavailable history/provider, denied scope, partial context budget, contradictory evidence preserved rather than hidden, protocol mismatch. Names/fields must be reconciled with existing public errors, not added as
...
Source-explainable retrieval, preserved contradictory claims, history independent of MVCC, project/global isolation, end-to-end backup/export recovery; all detailed AGENT-6..8 quality/security/observability gates.
