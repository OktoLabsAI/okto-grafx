...
__all__ = ["ProjectionLimits", "ProjectionNode", "ProjectionEdge", "GraphProjection", "project_graph"]
...
class ProjectionLimits:
    ...
    def __post_init__(self) -> None:
        for field in ("max_nodes", "max_edges", "max_memory_bytes", "max_work"):
...
def project_graph(database: Database, reader: Transaction | None = None, *,
    ...
    results and the scan engine's one-row decode buffers. No hidden full-table query.
    """
    ...
    for table in definitions:
        ...
        while True:
            work.step()
            page = reader.scan_rows_v1(table.name, limit=1, cursor=cursor)
