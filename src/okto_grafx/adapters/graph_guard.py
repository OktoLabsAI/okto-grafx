"""The guard the vector engine publishes its derived graph under, built on ``threading``.

The engine's ``GraphGuard`` protocol asks for a context manager that can wait and wake, plus the
identity of the calling thread -- the one thing a bare ``threading.Condition`` cannot say, and
the one thing that tells a builder's own re-entrant call apart from another thread's search.
This is mechanism, so it lives with the adapters and reaches the engine by construction (CF-13):
the pure core imports no ``threading``.
"""
