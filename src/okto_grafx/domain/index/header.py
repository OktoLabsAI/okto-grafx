...
POSTING_INDEX_HEADER_FORMAT_VERSION: int = 5
"""Exact page-local dictionary/reference postings; unchanged tagged header grammar."""
...
class IndexHeader:
    ...
    def __post_init__(self) -> None:
        ...
        elif self.format_version == POSTING_INDEX_HEADER_FORMAT_VERSION:
            if self.layout is not IndexLayout.POSTING_HASH or self.visibility is not IndexVisibility.EXACT:
                raise GrafxIndexError("Index header format 5 requires exact posting hash.", field="layout")
    ...
    def encode(self) -> bytes:
        ...
        if self.format_version in (ORDERED_INDEX_HEADER_FORMAT_VERSION, SPARSE_INDEX_HEADER_FORMAT_VERSION, POSTING_INDEX_HEADER_FORMAT_VERSION):
    ...
    def decode(cls, raw: bytes) -> IndexHeader:
        ...
        if format_version > POSTING_INDEX_HEADER_FORMAT_VERSION:
            raise GrafxSchemaVersionMismatch(
                f"This build reads index format {POSTING_INDEX_HEADER_FORMAT_VERSION} and "
                "below; the "
        ...
        if format_version in (ORDERED_INDEX_HEADER_FORMAT_VERSION, SPARSE_INDEX_HEADER_FORMAT_VERSION, POSTING_INDEX_HEADER_FORMAT_VERSION):
            if format_version in (SPARSE_INDEX_HEADER_FORMAT_VERSION, POSTING_INDEX_HEADER_FORMAT_VERSION) and (
